...
public class Targeting2d {
  ...
  public static Optional<Pose2d> getTarget(String name, int nearestTagId) {
      return Optional.ofNullable(m_aprilTagList.get(nearestTagId)).map(data -> data.getRelativeTargetPose(name)).orElse(Optional.ofNullable(m_targetList.get(name).getPose()));
  }
