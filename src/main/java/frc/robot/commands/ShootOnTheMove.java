...
public class ShootOnTheMove extends Command {
  ...
  private final SwerveSubsystem m_swerveDrivetrain;
  ...
  private Rotation2d launcherRotation = new Rotation2d(Degrees.zero());
  private Transform2d robotToLauncher =
      new Transform2d(Meters.zero(), Meters.zero(), launcherRotation);
  ...
  public void initialize() {
    ...
    m_PidController =
        new ProfiledPIDController(
            ...
            kTeleD_Theta,
            new Constraints(
                SWERVE.kMaxSpeed.in(MetersPerSecond),
                SWERVE.kMaxAccelerationShooting.in(MetersPerSecondPerSecond)));
  ...
  public void execute() {
    ...
    // Simplified with CTRE SwerveRequest
    m_swerveDrivetrain.setChassisSpeedsWithHeading(
        SWERVE.kMaxSpeedShooting.times(m_throttleInput.getAsDouble()),
        SWERVE.kMaxSpeedShooting.times(m_strafeInput.getAsDouble()),
  ...
  public void end(boolean interrupted) {
    m_flywheel.setTorqueCurrentOutputFOC(Volts.zero());
  }
