...
public class Vision extends SubsystemBase {
  ...
  public static class VisionFieldPoseEstimate {
  ...
  public void periodic() {
    ...
    if (llfSuccess && llrSuccess && lllSuccess) {
      ...
      m_swerveDriveTrain.addVisionMeasurement(
          fuseEstimates(LLF.getFieldPoseEstimate(), LLL.getFieldPoseEstimate()));
    } else if (llrSuccess && lllSuccess) {
      m_swerveDriveTrain.addVisionMeasurement(
          fuseEstimates(LLR.getFieldPoseEstimate(), LLL.getFieldPoseEstimate()));
    } else if (llfSuccess) {
      m_swerveDriveTrain.addVisionMeasurement(LLF);
    } else if (llrSuccess) {
      m_swerveDriveTrain.addVisionMeasurement(LLR);
    ...
    if (!matchStarted) {
      if (llfSuccess)
        visionAngle =
            avgStartingAngle.calculate(LLF.getLastGoodEstimate().pose.getRotation().getDegrees());
      if (llrSuccess)
        visionAngle =
            avgStartingAngle.calculate(LLR.getLastGoodEstimate().pose.getRotation().getDegrees());
      if (lllSuccess)
        visionAngle =
            avgStartingAngle.calculate(LLL.getLastGoodEstimate().pose.getRotation().getDegrees());
    }
  }
