...
public class Climber2d {
    private Color8Bit bottomClimberColor = new Color8Bit(255, 0, 102); // Rose pink color for bottom climber
    private Color8Bit bottomClimberHookColor = new Color8Bit(127, 0, 51); // Darker rose pink color for bottom climber hook
    private Color8Bit topClimberColor = new Color8Bit(255, 0, 255); // Pure magenta color for top climber
    private Color8Bit topClimberHookColor = new Color8Bit(127, 0, 127); // Darker magenta color for top climber hook
    ...
    public Climber2d(MechanismRoot2d parentRoot) {
        m_climberBottomCarriage = new Elevator2d(
        ...
        m_climberTopCarriage = new Elevator2d(
                new Elevator2dConfig(
                        "Climber_topCarriage",
                        topClimberColor,
    ...
    public void update(Distance height, LinearVelocity velocity) {
        ...
        if ((CLIMBER.upperLimit.minus(height)).in(Inches) >= 1.25) {
            ...
            bottomHookAngle = -180.0 + (45.0 * (1.25 - (CLIMBER.upperLimit.minus(height).in(Inches) / 1.25)));
        }
