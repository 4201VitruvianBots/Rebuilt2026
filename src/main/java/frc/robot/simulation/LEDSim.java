    ...
    public LEDSim(AddressableLEDBuffer ledBuffer, Layout layout) {
        ...
        if (layout == Layout.HORIZONTAL) {
            for (int i = 0; i < ledBuffer.getLength(); i++) {
                m_ledLigaments[i] = m_mech2d.getRoot("LED " + i, i * 10, 5).append(new MechanismLigament2d("LED " + i, 10, 0));
            }
        } else {
            for (int i = 0; i < ledBuffer.getLength(); i++) {
                m_ledLigaments[i] = m_mech2d.getRoot("LED " + i, 5, i * 10).append(new MechanismLigament2d("LED " + i, 10, 0));
            }
        }
